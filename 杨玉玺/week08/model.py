...
class SentenceEncoder(nn.Module):
    ...
    """
        x.gt(0)
    gt 是 greater than 的缩写，x.gt(0) 是一个逐元素比较操作，它会将输入张量 x 中的每个元素与 0 进行比较。
    如果元素大于 0，则对应位置的结果为 True；如果元素小于等于 0，则对应位置的结果为 False。
    返回的是一个与 x 形状相同的布尔型张量。

        torch.sum
        函数用于对输入张量的指定维度求和。
        dim = -1
        表示在最后一个维度上进行求和操作。在句子编码的场景中，最后一个维度通常对应句子的长度维度。
        由于布尔型张量在进行求和操作时，True
        会被当作
        1，False
        会被当作
        0，所以求和的结果就是每个样本中大于
