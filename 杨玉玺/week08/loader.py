...
class DataGenerator:
    ...
    def random_train_sample(self):
        standard_question_index = list(self.knwb.keys())
        # 选取两个意图，之后从第一个意图中取2个问题，第二个意图中取一个问题
        x, y = random.sample(standard_question_index, 2)
        #如果选取到的标准问下不足两个问题，两个正样本是一样的
        if len(self.knwb[x]) == 1:
            s1 = s2 = self.knwb[x][0]  # 两个正样本选择一样的
        else:
            s1, s2 = random.sample(self.knwb[x], 2)
        # 随机一个负样本
        s3 = random.choice(self.knwb[y])
        # 前两个相似，后一个不相似
        return [s1, s2, s3]
